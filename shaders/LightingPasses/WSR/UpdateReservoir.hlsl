...
void main(uint3 GlobalIndex : SV_DispatchThreadID, uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID)
{
    ...
    uint targetId = clamp(floor(RAB_GetNextRandom(rng) * sampleCnt), 0, sampleCnt - 1) + stats.offset;
    WSRLightSample wsrLightSample = t_WorldSpaceReorderedLightSamplesBuffer[targetId];
    uint2 reservoirPosition = uint2(wsrLightSample.reservoirPosition >> 16, wsrLightSample.reservoirPosition & 0xFFFF);

    uint2 pixelPosition = RTXDI_ReservoirPosToPixelPos(reservoirPosition, g_Const.runtimeParams.activeCheckerboardField);
    RAB_Surface targetSurface = RAB_GetGBufferSurface(pixelPosition, false);
    RTXDI_DIReservoir targetReservoir = RTXDI_LoadDIReservoir(g_Const.restirDI.reservoirBufferParams, reservoirPosition, g_Const.restirDI.bufferIndices.shadingInputBufferIndex);

    RTXDI_DIReservoir state = RTXDI_EmptyDIReservoir();
    state.canonicalWeight = 0.0f;

    uint updateSampleCnt = 2;//WORLD_SPACE_LIGHT_SAMPLE_NUM_PER_RESERVOIR;
    for (uint i = 0; i < updateSampleCnt; ++i)
    {
        uint sampleIndex = clamp(floor(RAB_GetNextRandom(rng) * sampleCnt), 0, sampleCnt - 1) + stats.offset;
        wsrLightSample = t_WorldSpaceReorderedLightSamplesBuffer[sampleIndex];
        uint2 neighborPosition = uint2(wsrLightSample.reservoirPosition >> 16, wsrLightSample.reservoirPosition & 0xFFFF);
        RAB_Surface neighborSurface = RAB_GetGBufferSurface(neighborPosition, false);
        RTXDI_DIReservoir neighborReservoir = RTXDI_LoadDIReservoir(g_Const.restirDI.reservoirBufferParams, neighborPosition, g_Const.restirDI.bufferIndices.shadingInputBufferIndex);

        RTXDI_StreamNeighborWithPairwiseMIS(state, RAB_GetNextRandom(rng),
            neighborReservoir, neighborSurface,
            targetReservoir, targetSurface,
            updateSampleCnt);
    }
    
    RTXDI_StreamCanonicalWithPairwiseStep(state, RAB_GetNextRandom(rng), targetReservoir, targetSurface);

    RTXDI_FinalizeResampling(state, 1.0, updateSampleCnt);
    
    if (!(g_Const.worldSpaceReservoirFlag & WORLD_SPACE_RESERVOIR_REUSE))
    {
        u_WorldSpaceLightReservoirs[reservoirIndex].packedSurface = PackWSRSurface(targetSurface);
    ...
    RTXDI_DIReservoir curReservoir = state;
    ...
    preReservoir.M = min(preReservoir.M, 20 * curReservoir.M);
    RTXDI_CombineDIReservoirs(state, curReservoir, 0.5f, curReservoir.targetPdf);
    ...
    RAB_LightSample preLightSample = RAB_SamplePolymorphicLight(
        ...
        targetSurface, RTXDI_GetDIReservoirSampleUV(preReservoir));

    float targetPdf = 0.f;
    if (RTXDI_IsValidDIReservoir(preReservoir))
    {
        targetPdf = RAB_GetLightSampleTargetPdfForSurface(preLightSample, targetSurface);
    }
